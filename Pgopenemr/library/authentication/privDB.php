...
// exit();
    ...
    {
        $secure_config=$GLOBALS['OE_SITE_DIR'] . "/secure_sqlconf1.php";
        // print_r($secure_config);
        // exit();
        // cek file ada atau tidak
        if(file_exists($secure_config)){
            require_once($secure_config);
            $GLOBALS[PRIV_DB]=NewADOConnection("postgres");
 ...
 * @return type
 */
 // $sqll="SELECT*FROM users_secure";
 // print_r($sqll);
 // exit();
 // $sqll='SELECT id,password,salt FROM users_secure WHERE username=?';
 // print_r($GLOBALS['last_mysql_error']);
 // exit();
