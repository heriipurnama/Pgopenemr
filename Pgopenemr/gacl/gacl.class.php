...
/**
  yang dirubah koneksi posgreSQL ke pgsqlconf
*/
	...
	function __construct($options = NULL) {
		...
		if (is_array($options)) {
			foreach ($options as $key => $value) {
                ...
                global $pgsqlconf, $disable_utf8_flag;
                $this->_db_host = $pgsqlconf["host"];
                $this->_db_user = $pgsqlconf["login"];
                $this->_db_password = $pgsqlconf["pass"];
                $this->_db_name = $pgsqlconf["dbase"];
