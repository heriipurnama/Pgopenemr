...
<?php
  //Encounter details are stored to javacript as array.
  $resultAA = sqlStatement("SELECT dicom_id from patient_data WHERE pid = '".$pid."'");
  $dicomArr = sqlFetchArray($resultAA);
  $dicomID = $dicomArr['dicom_id'];
  /*echo $dicomID;
  print_r(sqlFetchArray($resultAA));*/
...
<table cellspacing='0' cellpadding='0' border='0' class="subnav">
  <tr>
      <td class="small" colspan='4'>
          ...
          <?php if($dicomID != ''){?>
          |
          <a href="../dicom/preview.php?id=<?php echo $dicomID;?>" onclick='top.restoreSession()'>
           <?php echo htmlspecialchars(xl('Dicom Viewer'),ENT_NOQUOTES); ?></a>
        <!--   |
          <a href="../../reports/external_data.php" onclick='top.restoreSession()'>
          <?php //echo xlt('Send Modality'); ?></a> -->
