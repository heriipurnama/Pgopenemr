...
function load_menu($menu_set){
    ...
    $menuTables="SELECT * FROM pg_catalog.pg_tables WHERE schemaname='public' ORDER BY tablename";
    //$menuTables="SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' LIKE ESCAPE ?";
    $res=sqlQuery($menuTables,array("menu_trees"));
    print_r(array("menu_trees"));
    exit();
    if($res===false){
        return array();
    }
