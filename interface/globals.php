...
// $glrow = sqlQuery("SELECT * FROM globals ");
$glrow = sqlQuery("SELECT c.relname AS Tables_in FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE pg_catalog.pg_table_is_visible(c.oid) AND c.relkind = 'r' AND relname LIKE 'glob%' ORDER BY 1;
");
// print_r($glrow);
// exit();
// echo "string".$glrow;
// $qglrow = json_decode('{"Tables_in_openemr (globals)":"globals"}',true); //rubah json jd array
// extract($data);
// print_r($qglrow);
// exit();
// $glrow = "Array ( [Tables_in_openemr (globals)] => globals )" ;
// $ok  = pg_query("SELECT*from users");
// $arr = pg_fetch_array($ok);
  ...
  if (!empty($_SESSION['authUserID'])) {
    ...
    if (!empty($_POST['authUser'])) {
      ...
      $temp_sql_ret = sqlQuery("SELECT id FROM users WHERE username = ?", array($_POST['authUser']) );
       // echo "temp_sql_ret: ".$temp_sql_ret;
       // exit();
